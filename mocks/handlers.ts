...
export const handlers = [
  ...
  rest.get(`${SUPABASE_URL}${SUPABASE_AUTH_USER_API}`, async (req, res, ctx) => {
    const token = req.headers.get("authorization")?.split("Bearer ")?.[1];

    if (token !== "valid") return res(ctx.status(401), ctx.json({ error: "Token expired" }));
  ...
  rest.delete(`${SUPABASE_URL}${SUPABASE_AUTH_ADMIN_USER_API}/*`, async (req, res, ctx) => {
